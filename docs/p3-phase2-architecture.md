...
本阶段增加“逻辑器件实例”，用于让 MCU/SoC 的 POWER、GPIO、DDR 等符号单元代表同一颗物理器件。DevicePack v1 以向前兼容的可选字段增加 SPICE 端口映射。
...
共享多单元身份和 L1/L2 数据已贯通。L3 现在可通过显式 `spiceBindings` 将多个 symbol unit 聚合为一个 SPICE 实例；虚构 STA100 测试运放已拆为 ANALOG/POWER 两个 unit，并验证只生成一行 `X1`。这不改变 MCU/SoC 没有 SPICE 模型时的能力标识，也不会把 SDK、IBIS 元数据宣称为固件或完整芯片仿真。
