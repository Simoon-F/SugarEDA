...
转换成功后可把结果绑定到画布中的具体逻辑器件实例。项目级面板会在保存、重新打开后继续检查，并报告缺少配置的可配置实例。复制、删除与撤销保持配置引用一致。

未来可以为获授权来源实现独立的 Adapter profile，把特定工具的导出结果转换到此 IR。每种输入格式必须保持独立解析器、资源上限和来源记录；不得扩展本解析器去执行 `dtc`、SDK 脚本或厂商二进制。
